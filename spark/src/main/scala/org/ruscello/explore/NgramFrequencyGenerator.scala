...
object NgramFrequencyGenerator extends JobConfiguration with GeneralUtility {
  ...
  def main(args: Array[String]) {
    val appName = "ngramFrequencyGenerator"
	 ...
	 val symPairData = keyedData.groupByKey.flatMap(v => {
	   ...
	   val symPairs = keyedAttrValues.flatMap(r => {
	     ...
	     for (i <- 1 to attrValues.length - 1) {
	       val preSym = getPercentileIndex(percentiles, attrValues(i-1)).toString()
	       val curSym = getPercentileIndex(percentiles, attrValues(i)).toString()
